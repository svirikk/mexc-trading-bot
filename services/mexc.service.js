...
class MexcService {
  ...
  /**
   * Ринковий вхід у позицію З ОДРАЗУ прикріпленим TP/SL — ОДНИМ атомарним
   * запитом, замість окремого market-ордера + окремого виклику
   * stoporder/place після.
   *
   * ІСТОРІЯ: окремий виклик stoporder/place (метод placeTpSl нижче) двічі
   * падав на реальних ордерах з різними помилками ([600], потім [5001]) —
   * документація цього ендпоінту виявилась ненадійною/суперечливою.
   * Знайдено робочий приклад у ДВОХ незалежних community SDK
   * (mexc-futures-sdk на TS, mexc-api-python) — обидва ставлять
   * stopLossPrice/takeProfitPrice ПРЯМО в order/create, включно з market-
   * ордером (type=5). Це й архітектурно краще: TP/SL кріпиться в той самий
   * момент, що й вхід, тому вікна "позиція відкрита, але без захисту" не
   * існує в принципі — нема окремого кроку, який може впасти ПІСЛЯ
   * відкриття позиції.
   *
   * КОМПРОМІС: на відміну від окремого stoporder/place (де можна було
   * явно попросити stopLossType=1/takeProfitType=1, тобто ЛІМІТНЕ
   * виконання після спрацювання), інлайн-спосіб у підтверджених прикладах
   * не показує такого поля — тобто TP/SL тут, найімовірніше, виконуються
   * як MARKET по спрацюванню (гарантоване виконання, без ризику
   * "зависання" ліміт-ордера на геп ціни, але можлива невелика негативна
   * сліпедж-різниця проти точної ціни).
   */
  async openMarketOrderWithProtection({
    symbol, side, vol, leverage, openType, price, positionMode,
  ...
  /**
   * ⚠️ БІЛЬШЕ НЕ ВИКОРИСТОВУЄТЬСЯ в основному потоці (index.js) — замінено
   * на openMarketOrderWithProtection() вище, яка кріпить TP/SL атомарно
   * разом із входом. Лишив метод тут як утиліту (напр. для ручного
   * скрипта "поставити/змінити TP/SL на вже відкритій позиції" в
   * майбутньому), АЛЕ ця конкретна комбінація параметрів двічі падала на
   * реальних ордерах з різними помилками ([600], потім [5001]) — перш ніж
   * покладатись на цей метод знову, варто спершу перевірити з підтримкою
   * MEXC (https://t.me/MEXCAPIsupport) чи на маленькій сумі окремо.
   *
